...
def clean_lyrics(df,column):
    ...
    """
    df = df
    df[column] = [i.lower() for i in df[column]]
    df[column] = [i.replace(r"verse |[1|2|3]|chorus|bridge|outro","").str.replace("[","").str.replace("]","") for i in df[column]]
    df[column] = [i.lower().str.replace(r"instrumental|intro|guitar|solo","") for i in df[column]]
    df[column] = [i.replace("\n"," ").str.replace(r"[^\w\d'\s]+","").str.replace("efil ym fo flah","") for i in df[column]]
    df[column] = [i.strip() for i in df[column]]
